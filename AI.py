...
class MyModel:
    ...
    def train_model(self, file, lr=0.00005, batch_size=512, epochs=300, stop_early=True, visualize_only=False):
        ...
        self.loaded_model.compile(loss=keras.losses.huber, optimizer=opt)
    ...
    def approximate_reward(self, data, action):
        return \
            self.loaded_model.call(tf.expand_dims(tf.convert_to_tensor((np.append(data, [action]))), axis=0)).numpy()[
                0][0]
...
def simulate(rounds, epsilon, discount_factor, thread_filename="-1"):
    ...
    for current_round in range(rounds):
        ...
        dataframe.loc[dataframe.eval(query), '49'] = dataframe.loc[dataframe.eval(query), '49'] * discount_factor * (
                    2 / 3) + dataframe.loc[dataframe.eval(query), '50'] * final_score * (
                                                                 2 / 3)  # final_score * dataframe.loc[dataframe.eval(query), '50'] #dataframe.loc[dataframe.eval(query), '49']*dataframe.loc[dataframe.eval(query), '50']*(2/3)+final_score*dataframe.loc[dataframe.eval(query), '50']*(2/3)   #final_score * dataframe.loc[dataframe.eval(query), '50']
    ...
    for i in range(rounds):
        ...
        if (multithreaded):
            results = multithread_simulation(threads=threads,
                                              ...
                                              epsilon=epsilon, discount_factor=discount_factor)
        else:
            results = simulate(simulations_per_round, epsilon, discount_factor)
        filename = results[0]
        my_plot.scores.append(results[1])
        if (
                draw_plot == True):  # TODO: Fix this in multithreading, right now the score object is not passed to child threads so scores are empty
            my_plot.draw_plot()
...
if __name__ == '__main__':
    ...
    simulate_and_train(rounds=100, simulations_per_round=120, starting_epsilon=0.4, starting_learning_rate=0.001,
                       draw_plot=True, epsilon_decay=False, lr_decay=False, discount_factor=0.85, epochs_per_round=1,
